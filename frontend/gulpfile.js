...
gulp.task('setupBootstrap', () => {
    ...
    gulp.src([nodepath + 'bootstrap/scss/*.scss']).pipe(gulp.dest(paths.sass + 'core/'));
    return gulp.src([nodepath + 'bootstrap/scss/**/*.scss']).pipe(gulp.dest(paths.sass + 'core/'));
  });
...
gulp.task('compile-pug', () => {
    return gulp.src(paths.dev +'markup/pages/**/*.pug')
      .pipe(pug({
        pretty: true
      }))
...
gulp.task('copy-css', () => {
    return gulp.src(paths.dev + 'assets/css/**/*.css')
        .pipe(gulp.dest(paths.public + 'assets/css/'))
        .on('finish', browser.reload);
});
...
gulp.task('copy-fonts', () => {
    return gulp.src(paths.dev + 'assets/fonts/*')
...
gulp.task('copy-js', () => {
    return gulp.src(paths.dev + 'assets/js/**/*.js')
...
gulp.task('copy-images', () => {
    return gulp.src(paths.dev + 'assets/images/**/*')
        .pipe(gulp.dest(paths.public + 'assets/images/'))
        .on('finish', browser.reload);
});

gulp.task('init', gulp.parallel('setupBootstrap'));
gulp.task('build', gulp.parallel('copy-images','copy-css','copy-js','copy-fonts','compile-js', 'compile-sass', 'compile-pug'),
  () => {done()});
gulp.task('default', gulp.series('reset', 'build', gulp.parallel('server', 'watch'),
  () => {done()}));
